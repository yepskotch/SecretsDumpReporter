...
- **Sensitive account detection** — flag privileged or service accounts using a regex pattern or file
...
- **CSV export** — full account list with reuse group, cracked password, blank password, LM hash and sensitive flags
...
| `-s / --sensitive` | Regex pattern or path to a file of patterns to flag sensitive accounts |
...
secretsdump-reporter dump.ntds -p hashcat.potfile -s "admin|svc_|krbtgt" -o results/report
secretsdump-reporter dump.ntds -p hashcat.potfile -s sensitive.txt -o results/report
```

The sensitive patterns file accepts one regex per line; lines starting with `#` are treated as comments:
